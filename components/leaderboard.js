...
export default function Leaderboard({ title, type, segmentId, ...props }) {
    ...
    return (
        <div className={`${props.className} rounded-lg bg-purple-800 shadow-xl pb-3`}>
            <h3 className="text-purple-200 font-bold mx-3 mb-3 pt-3 pb-3 border-b-2 border-purple-500">{title}</h3>
            <div className="space-y-4">
            { data.length ?
                data.map(row => (
                    <div key={`${row.user_id}${row.segment_id}`} className="flex items-center space-x-3 px-4 py-1 text-md hover:bg-purple-700">
